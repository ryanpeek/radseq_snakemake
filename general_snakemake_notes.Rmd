...
```
snakemake -j 16 --use-conda --rerun-incomplete --latency-wait=15 --resources mem_mb=200000 --cluster "sbatch -t {cluster.time} -p high -N {cluster.nodes} -J {cluster.job-name} --mail-user={cluster.mail-user} -o {cluster.output} -e {cluster.error}" --cluster-config cluster_config.yml -k -p -r -n
```

```
snakemake -j 16 --use-conda --resources mem_mb=100000 --cluster "sbatch -t {cluster.time} -p high -N {cluster.nodes} -J {cluster.job-name} --mail-user={cluster.mail-user} --mail-type={cluster.mail-type -o {cluster.output} -e {cluster.error}" --latency-wait=15 --cluster-config ~/.config/snakemake/slurm/cluster_config.yml -k -p -r -n
```

```
snakemake -j 16 --use-conda --resources mem_mb=100000 --cluster "sbatch -t {cluster.time} -p high -N {cluster.nodes} -J {cluster.job-name} --mail-user={cluster.mail-user} --mail-type={cluster.mail-type -o {cluster.output} -e {cluster.error}" --latency-wait=15 --cluster-config ~/.config/snakemake/slurm/cluster_config.yml -k -p -r
```

```
snakemake -j 16 --use-conda --rerun-incomplete --latency-wait 15 --resources mem_mb=500000 --cluster "sbatch -t 1440 -J pca -p high -n 1 -N 1 -c 1 --mail-user=rapeek@ucdavis.edu -o slurms/pca_%j.out -e slurms/pca_%j.err" -k -p -r -n
```