...
```
snakemake -j 16 --use-conda --cluster "sbatch -t {cluster.time} -p {cluster.partition} -N {cluster.nodes}" --latency-wait=15	--profile slurm_farm --cluster-config cluster_config.yml -k -p -r -n
```


```
snakemake -j 16 --use-conda --cluster "sbatch -t 1440 -J pca -p high -n 1 -N 1 -c 1 --mail-user=rapeek@ucdavis.edu -o slurms/pca_%j.out -e slurms/pca_%j.err" -k -p -r
```
