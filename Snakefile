...
rule all:
    input: 
        expand("outputs/well_split_fastq/{lane}_{plate}_R{read}_{sample}.fastq", lane = LANES, plate = PLATES, sample = SAMPLES, read = READS)
...
rule plate_split_fastq:
    ...
    output: "outputs/plates_split_fastq/{lane}_{plate}_R{read}.fastq"
    shell:"""
...
rule well_split_fastq:
    input: expand("outputs/plates_split_fastq/{{lane}}_{{plate}}_R{read}.fastq", read = READS)
    output: expand("outputs/well_split_fastq/{{lane}}_{{plate}}_R{read}_{sample}.fastq", sample = SAMPLES, read = READS)
    params: outdir = "outputs/well_split_fastq"
    shell:"""
