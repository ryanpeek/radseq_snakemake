...
rule all:
    input: 
        expand("outputs/well_split_fastq/{lane}_{plate}_{sample}_R{read}.fastq", lane = LANES, plate = PLATES, sample=SAMPLES, read = READS)
...
rule plate_split_fastq:
    ...
    output: "outputs/plate_split_fastq/{lane}_{plate}_R{read}.fastq" 
    shell:"""
...
rule well_split_fastq:
    input: expand("outputs/plate_split_fastq/{{lane}}_{{plate}}_R{read}.fastq", read = READS)
    output: expand("outputs/well_split_fastq/{{lane}}_{{plate}}_{sample}_R{read}.fastq", sample = SAMPLES, read = READS)
